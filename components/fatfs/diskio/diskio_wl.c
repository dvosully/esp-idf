...
DRESULT ff_wl_read (BYTE pdrv, BYTE *buff, DWORD sector, UINT count)
{
    ...
    if (unlikely(err != ESP_OK)) {
        ESP_LOGE(TAG, "wl_read failed %s", esp_err_to_name(err));
        return RES_ERROR;
    }
    return RES_OK;
}
...
DRESULT ff_wl_write (BYTE pdrv, const BYTE *buff, DWORD sector, UINT count)
{
    ...
    if (unlikely(err != ESP_OK)) {
        ESP_LOGE(TAG, "wl_erase_range failed %s", esp_err_to_name(err));
        return RES_ERROR;
    }
    err = wl_write(wl_handle, sector * wl_sector_size(wl_handle), buff, count * wl_sector_size(wl_handle));
    if (unlikely(err != ESP_OK)) {
        ESP_LOGE(TAG, "wl_write failed %s", esp_err_to_name(err));
        return RES_ERROR;
    }
    return RES_OK;
}
